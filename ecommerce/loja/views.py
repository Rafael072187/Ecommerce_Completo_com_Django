...
# mantém na memória os últimos IDs processados
ULTIMOS_PROCESSADOS = set()

@csrf_exempt
def finalizar_pagamento(request):
    """Recebe notificações do Mercado Pago e evita repetições."""
    if request.method == "POST":
        try:
            ...
            # trava: ignora se já processou esse pagamento
            if not payment_id or payment_id in ULTIMOS_PROCESSADOS:
                return HttpResponse(status=200)

            ULTIMOS_PROCESSADOS.add(payment_id)
            print(f"🔔 Notificação Mercado Pago recebida: payment_id={payment_id}, user_id={user_id}, topic={topic}")

            # só processa se existir um pagamento no sistema
            pagamento = Pagamento.objects.filter(id_pagamento=payment_id).first()
            if pagamento and not pagamento.aprovado:
                ...
                print(f"✅ Pedido {pedido.id} finalizado e e-mail enviado.")
    ...
    # fluxo normal (GET) — quando o usuário volta do checkout
    params = request.GET.dict()
    if params.get("status") == "approved":
        pagamento = Pagamento.objects.filter(id_pagamento=params.get("preference_id")).first()
        if pagamento and not pagamento.aprovado:
            pagamento.aprovado = True
            pedido = pagamento.pedido
            pedido.finalizado = True
            ...
            pagamento.save()
            enviar_email_compra(pedido)
            print(f"✅ Retorno via navegador: pedido {pedido.id} aprovado.")
        return redirect(f"/pedido_aprovado/{pagamento.pedido.id}/")
